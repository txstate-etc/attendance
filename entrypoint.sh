...
perl -i -pe 's/\Q{{WEB_HOSTNAME}}\E/$ENV{WEB_HOSTNAME}/' /etc/apache2/apache2.conf /tmp/docker/config/environments/production.rb
...
cd /tmp/docker && runuser -u www-data rake assets:precompile
